...
@ProviderFor(PendingSetDeletions)
final pendingSetDeletionsProvider = PendingSetDeletionsProvider._();
...
/// the rows on screen, so they close up the moment one disappears and open
/// again when it comes back.
final class PendingSetDeletionsProvider
    extends $NotifierProvider<PendingSetDeletions, Set<String>> {
  /// Sets that have been swiped away but can still be brought back.
  ///
  /// Undo is done by delaying the delete rather than by putting a copy back
  /// afterwards: for five seconds the row is only hidden from the table, and
  ...
  /// the rows on screen, so they close up the moment one disappears and open
  /// again when it comes back.
  PendingSetDeletionsProvider._()
    : super(
        from: null,
        argument: null,
        retry: null,
        name: r'pendingSetDeletionsProvider',
        isAutoDispose: false,
        dependencies: null,
        $allTransitiveDependencies: null,
      );
  ...
  @override
  String debugGetCreateSourceHash() => _$pendingSetDeletionsHash();

  @$internal
  @override
  PendingSetDeletions create() => PendingSetDeletions();
...
String _$pendingSetDeletionsHash() =>
    r'a74cacd88af8f97f298cfb0c385cca34cede304f';
...
abstract class _$PendingSetDeletions extends $Notifier<Set<String>> {
  Set<String> build();
  @$mustCallSuper
  @override
