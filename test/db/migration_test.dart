...
void main() {
  ...
  test('opening a v1 database migrates it to the current version', () async {
    ...
    final raw = sqlite3.open(file.path);
    expect(raw.select('PRAGMA user_version').first.values.first, 28);
    raw.close();
  });
  ...
  test('no user data is lost', () async {
    ...
    // v28: materiaal mee tekenen is aan zolang niemand iets koos, want dat is
    // wat elke tekening tot nu toe probeerde.
    expect(settings.imageEquipment, isNull);
    ...
    // v25: wat de coach aanbiedt toe te voegen, tot je erop tikt.
    expect(
      chatColumns.map((row) => row.read<String>('name')),
      contains('proposals'),
    );
